...
export function updateQueueStatusByStudentID(id, time, status) {
  const statement =
...
export function requestQueue(idMahasiswa, idDosen) {
  const data = {
    idMahasiswa,
    idDosen,
    waktu: Date.now(),
...
export function cancelQueue(time, studentID, professorID) {
  const statement =
...
export function getQueueByProfessorID(professorID) {
  const statement =
...
export function getProfessorsByStudyProgram(id) {
  const statement =
